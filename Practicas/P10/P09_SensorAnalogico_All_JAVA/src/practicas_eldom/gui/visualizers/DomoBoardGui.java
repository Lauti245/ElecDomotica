...
public class DomoBoardGui extends JPanel implements Visualizer {
	...
	public DomoBoardGui(String category, int address, CommTransport sn_Transport) {
		...
		panel_3_1.setBorder(new BevelBorder(BevelBorder.LOWERED, new Color(0, 0, 255), new Color(0, 255, 0), Color.BLUE, Color.MAGENTA));
		panel_3_1.setBounds(424, 305, 254, 206);
		add(panel_3_1);
		
		JLabel lblPersiana = new JLabel("Persiana");
		lblPersiana.setHorizontalAlignment(SwingConstants.CENTER);
		lblPersiana.setFont(new Font("Times New Roman", Font.PLAIN, 20));
		lblPersiana.setBounds(10, 172, 237, 24);
		panel_3_1.add(lblPersiana);
		
		JButton subida = new JButton("SUBIR");
		subida.setBounds(10, 86, 85, 31);
		subida.addMouseListener(new MouseListener() {
		    @Override
		    public void mouseClicked(MouseEvent e) {
		    ...
		    public void mouseReleased(MouseEvent e) {
		    	...
		    	buildModBus(address, Const_Modbus.WRITE_COIL, MB_Registers.MB_Discrete_Output_Coils.MB_ONOFF.getReg(), 0, Cregs);
		        // Aquí puedes añadir cualquier código que desees ejecutar cuando el mouse se suelte.
		        // Por ejemplo, podrías llamar a otra función o hacer alguna otra acción relacionada.
		    }

		    @Override
		    public void mousePressed(MouseEvent e) {
		    	
		    	buildModBus(address, Const_Modbus.WRITE_COIL, MB_Registers.MB_Discrete_Output_Coils.MB_ONOFF.getReg(), 1, Cregs);
		        // Este método se debe implementar pero no necesitas añadir código aquí si no quieres.
		    }
		...
		btnBajar.setBounds(10, 127, 85, 29);
		btnBajar.addMouseListener(new MouseListener() {
		    @Override
		    public void mouseClicked(MouseEvent e) {
		    ...
		    @Override
		    public void mouseReleased(MouseEvent e) {
		    	buildModBus(address, Const_Modbus.WRITE_COIL, MB_Registers.MB_Discrete_Output_Coils.MB_PWR.getReg(), 0, Cregs);
		    ...
		    public void mousePressed(MouseEvent e) {
		        // Este método se debe implementar pero no necesitas añadir código aquí si no quieres.
		    	 buildModBus(address, Const_Modbus.WRITE_COIL, MB_Registers.MB_Discrete_Output_Coils.MB_PWR.getReg(), 1, Cregs);
		    }
		...
		panel_3_1.add(btnBajar);
		
		estadoPersiana = new JLabel();
		estadoPersiana.setHorizontalAlignment(SwingConstants.CENTER);
		estadoPersiana.setFont(new Font("Times New Roman", Font.PLAIN, 20));
		estadoPersiana.setBounds(10, 10, 237, 24);
		panel_3_1.add(estadoPersiana);
		
		percentage = new JLabel();
		percentage.setHorizontalAlignment(SwingConstants.CENTER);
		percentage.setFont(new Font("Times New Roman", Font.PLAIN, 20));
		percentage.setBounds(10, 40, 237, 24);
		panel_3_1.add(percentage);
		
		JButton llave = new JButton("KEY");
		llave.setFont(new Font("Rubik", Font.PLAIN, 25));
		llave.setBounds(142, 78, 102, 92);
		llave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {				
				buildModBus(address, Const_Modbus.WRITE_COIL, 
						MB_Registers.MB_Discrete_Output_Coils.MB_KEY.getReg(), TSwitchState.ToNumber(TSwitchState.ON), Cregs);
			}
		});
		...
		case Const_Modbus.READ_INPUT_REGISTERS:
			...
			for(int i = addr;i<(addr+nReg); i++){
				...
				case MB_PERC:
					...
					percentage.setText(Integer.toString(e.getRegs()[i-addr])+"%");
					break;
				case MB_STATE:
					int state = (e.getRegs()[i-addr]);
					if(state == 3)
					estadoPersiana.setText("SUBIENDO");
					else if(state == 2) 
						estadoPersiana.setText("BAJANDO");
					else 
						estadoPersiana.setText("QUIETA");	
					break;
