...
void writecoil() {
	for (int addrReg = 0; addrReg < MB_O_COILS; addrReg++) {
		...
		case MB_TURNP:
			if(Cregs[addrReg]!=0x00 ||  mbDomoboard.BOTON1.Sensor->valor == 0 ){
				mbDomoboard.BOTON1.Aux = Cregs[addrReg] ;
				//if(mbDomoboard.BOTON1.Aux > 0x00  )mbDomoboard.setActuator(&(domoboard.PWRP), ON);
		...
		case MB_PWRP:
			//if(mbDomoboard.BOTON2.Aux>0  )mbDomoboard.setActuator(&(domoboard.PWRP), ON);

			if(Cregs[addrReg]!=0x00 ||  mbDomoboard.BOTON2.Sensor->valor == 0 ){
			mbDomoboard.BOTON2.Aux =  Cregs[addrReg]  ;
			}
			break;
		case MB_KEY:
			if(Cregs[addrReg]!=0x00 || mbDomoboard.BOTON1.Sensor->valor == 0){
...
//	DEBUGLOGLN("PIR activado: %d", Cregs[MB_ACTPIR]);
	...
	Aregs[MB_TMP_PIR] = ((EEPROM.read(ADDR_TIEMPO_PIR_1) & 0xFF) << 8)
			+ (EEPROM.read(ADDR_TIEMPO_PIR_2) & 0xFF);
