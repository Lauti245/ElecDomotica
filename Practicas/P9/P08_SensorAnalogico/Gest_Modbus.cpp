...
void writeholdingregister(){
	...
	for (int addrReg = 0; addrReg < MB_A_REGS; addrReg++) {
		...
		case MB_MINFOTOR:
			if(Aregs[MB_MINFOTOR]!=((EEPROM.read(ADDR_TIEMPO_MINFOTOR_1)&0xFF) << 8) + (EEPROM.read(ADDR_TIEMPO_MINFOTOR_2)&0xFF))
				{
				DEBUGLN("Actualizado");
				}
			EEPROM.update(ADDR_TIEMPO_MINFOTOR_1, (Aregs[MB_MINFOTOR]>>8)&0xFF);
			EEPROM.update(ADDR_TIEMPO_MINFOTOR_2, (Aregs[MB_MINFOTOR]&0xFF));

			break;
		case MB_MAXFOTOR:
			if(Aregs[MB_MAXFOTOR]!=((EEPROM.read(ADDR_TIEMPO_MAXFOTOR_1)&0xFF) << 8) + (EEPROM.read(ADDR_TIEMPO_MAXFOTOR_2)&0xFF))DEBUGLN("Actualizado");
			EEPROM.update(ADDR_TIEMPO_MAXFOTOR_1, (Aregs[MB_MAXFOTOR]>>8)&0xFF);
		    EEPROM.update(ADDR_TIEMPO_MAXFOTOR_2, (Aregs[MB_MAXFOTOR]&0xFF));
			break;
		case MB_MINFOTOT:
			if(Aregs[MB_MINFOTOT]!=((EEPROM.read(ADDR_TIEMPO_MAXFOTOT_1)&0xFF) << 8) + (EEPROM.read(ADDR_TIEMPO_MAXFOTOT_2)&0xFF))DEBUGLN("Actualizado");
			EEPROM.update(ADDR_TIEMPO_MAXFOTOT_1, (Aregs[MB_MINFOTOT]>>8)&0xFF);
			EEPROM.update(ADDR_TIEMPO_MAXFOTOT_2, (Aregs[MB_MINFOTOT]&0xFF));
			break;
		}
	}
}
...
/*
	...
	DEBUGLOGLN("Tiempo Max del fotoR: %d Segundos", Aregs[MB_MAXFOTOR]);
	DEBUGLOGLN("Tiempo Min del PIR: %d Segundos", Aregs[MB_MINFOTOR]);*/
}
