...
void Config_P7_SensorMovimiento(){
	...
	mbDomoboard.PIR_MOV.Sensor->Activo = false;		//Habilitamos sensor PIR
	Cregs[MB_ACTPIR] = 0x00;  						//Actualiza registro que monitoriza la habilitación del sensor
	...
	mbDomoboard.BTN_OPT.mbActuators.push(&(mbDomoboard.TRIAC));

	mbDomoboard.FOTOR.mbSensorEvent = checkearLuminosidad;
	mbDomoboard.FOTOR.mbActuators.push(&(mbDomoboard.RELE));

	mbDomoboard.FOTOT.mbSensorEvent = checkearLuminosidad;
	mbDomoboard.FOTOT.mbActuators.push(&(mbDomoboard.TRIAC));

	mbDomoboard.TEMP.mbSensorEvent = checkearTemperatura;
