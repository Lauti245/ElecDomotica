...
void ModbusSlave::set_BankInputs(uint16_t *Regs, uint16_t lengthRegs){
	DRegs = Regs;
	N_DRegs = lengthRegs;
}
...
*/
	...
	if (FC_WRITE_COIL == data[FUNC]) {
		...
		if (regs_num >= N_CRegs){
			//char msg[50];
			//sprintf(msg,"regs_num : %d --> N_ARegs : %d", regs_num, N_CRegs);
			//Serial.println(msg);
			return EXC_ADDR_RANGE;
		}
		return 0;
...
int ModbusSlave::write_single_register(unsigned int write_addr, unsigned char *query, uint16_t *regs)
{
        ...
        /*
        if(writesingleregister != NULL)
        	writesingleregister(write_addr, OP_ANALOG);
		*/
...
void ModbusSlave::configure(unsigned char slave, long baud, char parity, byte txenpin)
{
	this->slave = slave;
