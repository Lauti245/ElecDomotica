...
const unsigned char fsupported[] = { FC_READ_COILS, FC_READ_REG_DI, FC_READ_REG_AO,FC_READ_REG_I, FC_WRITE_COIL,
...
void ModbusSlave::set_BankInputHoldingRegister(uint16_t *Regs, uint16_t lengthRegs){
	IRegs = Regs;
	N_IRegs = lengthRegs;
}
/*
...
int ModbusSlave::read_holding_registers(unsigned char funcion ,unsigned int start_addr,unsigned char reg_count, uint16_t *regs)
{
	...
	build_read_packet(funcion, reg_count, packet);
...
int ModbusSlave::update()
{
	...
	case FC_READ_REG_AO:
		return read_holding_registers(FC_READ_REG_AO,start_addr,query[REGS_L],ARegs);
		break;
	case FC_READ_REG_I:
		return read_holding_registers(FC_READ_REG_I,start_addr,query[REGS_L],IRegs);
		break;
